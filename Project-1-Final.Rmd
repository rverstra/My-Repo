...
author: "Hudson Finch-Batista, Isidoro Villamayor, Robert Verstraete"
date: "8/8/2020"
...
## Mean Venture Funding/Mean Funding Rounds by Month Founded & Mean Funding Rounds by Funding Amount
```{r}
...
ggplot(venture_fund_month) + geom_col(mapping = aes(x = founded_month, y = mean_funding)) + coord_flip() + scale_x_discrete("Month") + scale_y_continuous("Mean Venture Funding") + ggtitle("Mean Venture Funding by Month") + theme(plot.title = element_text(hjust = 0.5))
...
ggplot(fund_rnd_month) + geom_col(mapping = aes(x = founded_month, y = mean_rnds))+ scale_x_discrete("Month") + scale_y_continuous("Mean Funding Rounds") + ggtitle("Mean Funding Rounds by Month") + theme(plot.title = element_text(hjust = 0.5)) + coord_flip()
...
op_ipo_market1 <- vc_data_clean2 %>% group_by(funding_rounds) %>% filter(status == "operating", !is.na(post_ipo_equity), post_ipo_equity >0) %>% summarise(mean_ipo_equity = mean(post_ipo_equity), max_ipo_equity = max(post_ipo_equity), min_ipo_equity = min(post_ipo_equity))
op_ipo_market1
...
ggplot(most_fund_global) + geom_col(mapping = aes(x =global_region, y = sum_funding)) + coord_flip()
